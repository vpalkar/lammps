...
FixBondBreakEdpd::FixBondBreakEdpd(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
...
FixBondBreakEdpd::~FixBondBreakEdpd()
{
  delete random;
...
void FixBondBreakEdpd::post_integrate()
{
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    double fractionT = fraction * exp( - 1.0 / ( (T[i] + T[j])/2) );
    printf("fraction = %f\n", fractionT);

    if (fraction < 1.0) {
      if (tag[i] < tag[j]) {
        if (probability[i] >= fractionT) continue;
      } else {
        if (probability[j] >= fractionT) continue;
      }
    }
...
void FixBondBreakEdpd::check_ghosts()
{
  int i,j,n;
  tagint *slist;
...
void FixBondBreakEdpd::update_topology()
{
...
void FixBondBreakEdpd::rebuild_special_one(int m)
{
...
void FixBondBreakEdpd::break_angles(int m, tagint id1, tagint id2)
{
  int j,found;
...
void FixBondBreakEdpd::break_dihedrals(int m, tagint id1, tagint id2)
{
  int j,found;
...
void FixBondBreakEdpd::break_impropers(int m, tagint id1, tagint id2)
{
  int j,found;
...
int FixBondBreakEdpd::dedup(int nstart, int nstop, tagint *copy)
{
  int i;
...
void FixBondBreakEdpd::post_integrate_respa(int ilevel, int /*iloop*/)
{
...
int FixBondBreakEdpd::pack_forward_comm(int n, int *list, double *buf,
...
void FixBondBreakEdpd::unpack_forward_comm(int n, int first, double *buf)
{
  int i,j,m,ns,last;
...
int FixBondBreakEdpd::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixBondBreakEdpd::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
double FixBondBreakEdpd::compute_vector(int n)
{
...
double FixBondBreakEdpd::memory_usage()
{
  int nmax = atom->nmax;
